...
def calculate_indicators(df):
    ...
    try:
        ...
        if len(existing_rename_map) < 3: # BBands 3개 컬럼 확인
             op_logger.warning(f"Bollinger Bands columns not fully generated. Available: {df_copy.columns.tolist()}")
             return None
...
# ==============================================================================
# 웹소켓 처리 로직 (K-line) - (WebSocket Processing Logic - K-line) - 수정됨
# ==============================================================================
...
def try_update_tp(sym_ws, sym_ccxt, side, amt, tp_id, tp_coid, cur_tp, new_tp):
    """(BBands TP) 조건 만족 시 기존 TP 주문을 취소하고 새로운 TP 주문을 설정"""
...
def process_kline_message(symbol_ws, kline_data):
    ...
    try:
        ...
        bbl, bbu = last.get('BBL', np.nan), last.get('BBU', np.nan) # BBands 값 (TP용)
        # StochRSI 컬럼 이름 생성 (파라미터 반영)
        stoch_k_col = f'STOCHRSIk_{STOCHRSI_LENGTH}_{STOCHRSI_RSI_LENGTH}_{STOCHRSI_K}_{STOCHRSI_D}'
        stoch_d_col = f'STOCHRSId_{STOCHRSI_LENGTH}_{STOCHRSI_RSI_LENGTH}_{STOCHRSI_K}_{STOCHRSI_D}'
        curr_k = last.get(stoch_k_col, np.nan) # 현재 %K 값
    ...
    if symbol_ws in pos_copy: # 해당 심볼 포지션이 존재하면
        ...
        if now - entry_time >= timedelta(minutes=POSITION_MONITORING_DELAY_MINUTES) and tp_id and current_tp_price:
            ...
            new_tp_target = bbu if side == 'long' else bbl # BBands 값 사용
        ...
        if not is_entry_attempted and not position_exists and not is_blacklisted:
            ...
            if tgt_side and tp_tgt is not None and tp_tgt > 0: # TP 가격 유효성 추가
                ...
                if expected_profit_ratio <= total_fee_ratio:
                    op_logger.info(f"[{symbol_ws}] Entry skipped: Expected profit ratio ({expected_profit_ratio:.4f}) is not greater than total fee ratio ({total_fee_ratio:.4f}). Initial TP (BB): {tp_tgt:.5f}, Entry: {entry_px:.5f}")
                    return
                # *** 수수료 체크 로직 끝 ***
                ...
                try:
                    op_logger.info(f"[{symbol_ws}] ===> Starting Entry Process: {tgt_side.upper()} @ {entry_px:.5f}, Initial TP Target (BB): {tp_tgt:.5f} (Profit Ratio {expected_profit_ratio:.4f} > Fee Ratio {total_fee_ratio:.4f})")
                    ...
                    # 사용 가능 잔고 조회 (여기서 다시 호출)
                    current_balance = get_current_balance()
                    if current_balance <= 0: # 0 이하 잔고는 진입 불가
                        raise Exception(f"Insufficient balance ({current_balance:.2f} {TARGET_ASSET}) for entry.")
                    ...
                    try:
                        ...
                        # TP 가격은 위에서 계산된 tp_tgt (BBands 값) 사용
