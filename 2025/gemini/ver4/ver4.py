...
# --- 수수료 설정 ---
FEE_RATE = 0.0005 # 예상 수수료율 (시장가 주문 및 TP 체결 시 편도 0.05% 가정)
...
TIMEFRAME_3M = '3m' # 포지션 모니터링용 3분봉 (MACD 반대 신호 시 종료용)
...
log_filename_base = "bot_log_v4" # 로그 파일 기본 이름
log_prefix = "[15m_MACDhist_BB_BBMtp_SL_V4.0]" # 로그 메시지 접두사 (전략 및 버전 명시)
...
def calculate_indicators(df):
    ...
    try:
        ...
        macd_hist_col_orig = f'MACDh_{MACD_FAST_PERIOD}_{MACD_SLOW_PERIOD}_{MACD_SIGNAL_PERIOD}'
...
def _create_conditional_order(symbol_ccxt, side, stop_price, amount, order_type, client_order_id_prefix, log_prefix_msg):
    ...
    try:
        ...
        if order_type == 'TAKE_PROFIT_MARKET' and e.code == -2021: # Order would immediately trigger
             ...
             return None # TP 주문 생성 실패로 간주하고 None 반환
        else:
...
def _check_macd_bb_entry_conditions(symbol_ws, idf):
    """
    진입 조건(MACD 히스토그램 및 BB 기준)을 확인하고 진입 방향, TP 목표, 진입가를 반환.
    롱: 현재가 < BB Low, MACD히스트(현재) > MACD히스트(이전). TP = BBM.
    숏: 현재가 > BB High, MACD히스트(현재) < MACD히스트(이전). TP = BBM.
    """
    if idf is None or idf.empty or len(idf) < 2: # Need at least 2 rows for previous MACD hist
    ...
    try:
        ...
        bb_low = last.get('BBL', np.nan)
        bb_high = last.get('BBU', np.nan)
        bb_mid = last.get('BBM', np.nan)
        
        current_macd_hist = last.get('MACD_hist', np.nan)
        prev_macd_hist = prev.get('MACD_hist', np.nan)

        if any(pd.isna(v) for v in [current_price, bb_low, bb_high, bb_mid, current_macd_hist, prev_macd_hist]):
            op_logger.debug(f"[{symbol_ws}] Required values for MACD Hist/BB entry check contain NaN. Price:{current_price}, BBL:{bb_low}, BBU:{bb_high}, BBM:{bb_mid}, MACD_H_curr:{current_macd_hist}, MACD_H_prev:{prev_macd_hist}")
            return None, None, None
    except IndexError:
        op_logger.debug(f"[{symbol_ws}] IndexError accessing indicator data for MACD Hist/BB entry.")
        return None, None, None
    except Exception as e:
        op_logger.error(f"[{symbol_ws}] Error accessing indicator data for MACD Hist/BB entry: {e}")
        return None, None, None
    ...
    if tgt_side and tp_tgt is not None and tp_tgt > 0 and entry_px > 0:
        ...
        if expected_profit_ratio <= total_fee_ratio:
            op_logger.info(f"[{symbol_ws}] {tgt_side.upper()} entry SKIPPED: Expected profit ratio ({expected_profit_ratio:.4%}) vs Total fee ratio ({total_fee_ratio:.4%}). TP:{tp_tgt:.5f}, Entry:{entry_px:.5f}")
            return None, None, None
        else:
            op_logger.info(f"[{symbol_ws}] {tgt_side.upper()} entry PROFITABILITY CHECK PASSED: Expected profit ratio ({expected_profit_ratio:.4%}) > Total fee ratio ({total_fee_ratio:.4%}).")
...
def _perform_entry_order(symbol_ws, sym_ccxt, tgt_side, entry_px, tp_tgt, now):
    ...
    try:
        ...
        min_notional_required = 5.0 # 일반적인 최소 주문 금액 (USDT)
        ...
        final_tp_price = tp_tgt # TP는 BBM 값으로 설정
        ...
        else: op_logger.warning(f"[{symbol_ws}] Take Profit (TP) order placement failed or returned no ID. Proceeding with SL only.")
        ...
        if 'sl_order' in locals() and sl_order and sl_order.get('id'): Thread(target=cancel_order, args=(sym_ccxt,), kwargs={'order_id': sl_order['id'], 'client_order_id': sl_order.get('clientOrderId')}, daemon=True).start()
        if 'tp_order' in locals() and tp_order and tp_order.get('id'): Thread(target=cancel_order, args=(sym_ccxt,), kwargs={'order_id': tp_order['id'], 'client_order_id': tp_order.get('clientOrderId')}, daemon=True).start()
...
def process_kline_message_main_tf(symbol_ws, kline_data):
    """메인 타임프레임 K-line 메시지를 처리하여 지표 계산, 진입 조건 확인 및 실행"""
    ...
    if is_closed:
        ...
        if not is_entry_attempted and not position_exists:
            ...
            tgt_side, tp_tgt, entry_px = _check_macd_bb_entry_conditions(symbol_ws, idf)

            if tgt_side and tp_tgt is not None and tp_tgt > 0 and entry_px > 0:
                _execute_entry_strategy(symbol_ws, sym_ccxt, tgt_side, entry_px, tp_tgt, now)
            elif tgt_side: # 조건은 맞았으나 TP/EntryPx가 유효하지 않은 경우
                op_logger.debug(f"[{symbol_ws}] Entry condition met for {tgt_side.upper()} but TP target ({tp_tgt}) or EntryPx ({entry_px}) is invalid. Skipping.")
...
if __name__ == "__main__":
    ...
    op_logger.warning("Strategy: 15m MACD Histogram & BB Entry / TP at BBM / Fixed SL / 3m MACD Opposite Signal Exit")
    ...
    op_logger.warning(f"             MaxPos={MAX_OPEN_POSITIONS}, SL={1-LONG_STOP_LOSS_FACTOR:.2%}/{SHORT_STOP_LOSS_FACTOR-1:.2%}, FeeRate={FEE_RATE*100:.3f}%")
    ...
    try:
        ...
        if kline_thread and kline_thread.is_alive(): kline_thread.join(timeout=5)
        if sync_thread and sync_thread.is_alive(): sync_thread.join(timeout=5)
        if symbol_update_thread and symbol_update_thread.is_alive(): symbol_update_thread.join(timeout=5)
        
        time.sleep(1) # Allow some time for ws close and thread joins
